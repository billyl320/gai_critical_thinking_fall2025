...
combined <- read.csv("combined_data_v4.csv")
rows_rmv <- c(1, 11)
combined <- combined[-rows_rmv, ]

# remove rows that column WA13.5 == 0

combined <- combined[combined$WA13.5 != 0.00, ]
  
combined$Final.Grade <- as.numeric(combined$Final.Grade)
...
## 1. boxplots
png("plots/boxplot_gai_hw_v4.png", width = 1024, height = 768)

boxplot(WA13.5 ~ Group,
        data = combined,
...
png("plots/boxplot_final_v4.png", width = 1024, height = 768)

boxplot(Final.Grade ~ Group,
        data = combined,
...
## 2. ANOVA table
fit <- aov(WA13.5 ~ Group + Instructor + Modality, data = combined)
summary(fit)
...
# Assumption 2
png("plots/qqplot_v4.png", width = 1024, height = 768, res = 200)
plot(fit, 2)
dev.off()

# Assumption 3 & 4
png("plots/residuals_fitted_plot_v4.png", width = 1024, height = 768, res = 200)
plot(fit, 1)
dev.off()
...
fit <- kruskal.test(WA13.5 ~ Group, data = combined)
fit
